...
const findValidPasswordResetTokens = `-- name: FindValidPasswordResetTokens :many
SELECT id, user_id, token_hash, expires_at, used, created_at FROM password_reset_token
WHERE used = false AND expires_at > now()
`

func (q *Queries) FindValidPasswordResetTokens(ctx context.Context) ([]PasswordResetToken, error) {
	rows, err := q.db.Query(ctx, findValidPasswordResetTokens)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
